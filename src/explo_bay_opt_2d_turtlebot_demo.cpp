...
//parameters for trajactory planning

vector<vector<pair<point3d, point3d>>> node_direct;
point3d current_node = point3d(0, 0, 0);
//vector<pair<point3d, point3d>> traject_direction;
double R_node = 3; //when no frontier in the drection, find frontier in R_node;
double Direct_reso = PI/6;
int traject_mode = 0; //1 for direction mode, 0 for circle mode, 2 for return to last node
bool base_origi = false;
...
vector<vector<point3d>> generate_frontier_points(const octomap::OcTree *octree, vector<vector<pair<point3d, point3d>>> node_direct) {

    vector<vector<point3d>> frontier_lines;
    vector<point3d> frontier_points;
    octomap::OcTreeNode *n_cur_frontier;

    vector<vector<point3d>> frontier_lines2;
    vector<point3d> frontier_points2;
    vector<pair<point3d,point3d>> New_branch;
    double Main_direct = node_direct[node_direct.size()-1][node_direct[node_direct.size()-1].size()-1].second.z();
    double frontier_angle;
    int n_rotate = 0; //time of go back to last base, and rotate direcion
    //int n_back = 0; //back how many nodes
      ...
      if(!cur_tree_2d->isNodeOccupied(*n))
        {
         ...
         for (double x_cur_buf = x_cur - 0.1; x_cur_buf < x_cur + 0.15; x_cur_buf += octo_reso)
            ...
            if(frontier_true)// && num_free >5 )
            {
                ...
                if (!node_direct.empty()&&traject_mode==1){
                    frontier_angle = atan2(x_frontier-current_node.y(), y_frontier-current_node.x());
                }

                double dist_frontier = sqrt(pow(x_frontier-current_node.y(),2) + pow(y_frontier-current_node.x(), 2));
                ...
                while(!node_direct.empty()&&traject_mode==1&&abs(frontier_angle - Main_direct)<= Direct_reso/2){
                ...
                // Frontier about laser_Orig by R_node
                while(dist_frontier<= R_node){
                    if(frontier_lines2.size() < 1)
                    {
                        frontier_points2.resize(1);
                        frontier_points2[0] = point3d(x_frontier,y_frontier,z_frontier);
                        frontier_lines2.push_back(frontier_points2);
                        frontier_points2.clear();
                    }
                    else
                    {
                        bool belong_old = false;            

                        for(vector<vector<point3d>>::size_type u = 0; u < frontier_lines2.size(); u++){
                                distance = sqrt(pow(frontier_lines2[u][0].x()-x_frontier, 2)+pow(frontier_lines2[u][0].y()-y_frontier, 2)) ;
                                if(distance < R1){
                                   frontier_lines2[u].push_back(point3d(x_frontier, y_frontier, z_frontier));
                                   belong_old = true;
                                   break;
                                }
                        }
    ...
    if(frontier_lines.size() < 2){ // if ther are few frontiers in the direction.
        if(frontier_lines2.size() > 2) {   
            frontier_lines.clear();
            frontier_lines = frontier_lines2;
            traject_mode = 0;
        }
        else {// both in direction and neighborhood are few points, try last node
            traject_mode = 3;
            n_rotate++;
            if(n_rotate = 0){

                New_branch = node_direct[node_direct.size()-1];
                New_branch.pop_back();
 
                node_direct.push_back(New_branch); // go back one node and generate new branch
                New_branch.clear();
                node_direct[node_direct.size()-1][node_direct[node_direct.size()-1].size()-1].second.z()+= Direct_reso;
                goto new_branch_back;
                ...
                node_direct.push_back(New_branch);
                node_direct[node_direct.size()-1][node_direct[node_direct.size()-1].size()-1].second.z()+= Direct_reso;
                New_branch.clear();
                n_rotate = 1;
                goto new_branch_back;
...
;
    ...
    while (ros::ok())
    {
            ...
            //do exploration if entropy is under 140
            if(entropy < 140){
                level = 1;
                frontier_lines = generate_frontier_points( cur_tree_2d, node_direct );

                if(!BayOpt) candidates = generate_candidates(frontier_lines, laser_orig, 0.4, 0.4, 6);
                else {
                    candidates = generate_candidates(frontier_lines, laser_orig, 0.4, 0.4, 6);
                }
            ...
            //for trajectory planning
            if(traject_mode == 0){
                //node_direct[node_direct.size()].first.push_back(point3d(laser_orig.x(), laser_orig.y(), laser_orig.z()));
                current_node = point3d(laser_orig.x(), laser_orig.y(), laser_orig.z());// the point without frontiers in direction will be a node
                base_origi = false;
            }
...
OS_INFO("%lu candidates generated.", candidates.size());
        ...
        {   
            ...
            if(level == 1) MIs[i] = calc_MI(cur_tree, c.first, hits, before)/pow(pow(c.first.x()-laser_orig.x(), 2)+pow(c.first.y() - laser_orig.y(), 2), 1.5);
        ...
        if(BayOpt){
            ...
            for(unsigned long int t = 1; t < 4; t++){
                ...
                if(level == 1) MIs_next = calc_MI(cur_tree, c.first, hits, before)/pow(pow(candidates_next.first.x()-laser_orig.x(), 2)+pow(candidates_next.first.y() - laser_orig.y(), 2), 1.5);
        ...
        if(arrived)
        {
            ...
            if(traject_mode == 0){
                double node_angle = atan2(laser_orig.y() - current_node.y(), laser_orig.x()-current_node.x());
                node_direct[node_direct.size()-1].push_back(make_pair<point3d,point3d>(point3d(current_node.x(), current_node.y(), current_node.z()), point3d(0, 0, node_angle)));
                current_node = point3d(laser_orig.x(), laser_orig.y(), laser_orig.z());
                //base_origi = false;
            }
